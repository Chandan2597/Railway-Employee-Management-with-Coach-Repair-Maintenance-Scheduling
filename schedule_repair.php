...
<title>Schedule Coach Repair</title>
	...
	<div class="table-responsive">
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					<div class="col-sm-6">
						<h2>Schedule Coach Maintenance</h2>
					</div>
					<div class="col-sm-6">
		...
		<table class="table table-striped table-hover">
			...
			<?php
				...
				$offset = ($page - 1) * $limit;
				$sql="SELECT * FROM repairschedule join coach on repairschedule.coachid=coach.coachid join admin on repairschedule.adminid=admin.adminid WHERE status='Pending' LIMIT {$offset},{$limit}";
				...
				?>
					...
					<td><?php echo $row['coachtype']; ?></td>
					<td><?php echo $row['age']; ?></td>
					<td><?php echo $row['color']; ?></td>
					<td><?php echo $row['date']; ?></td>
					<td><?php echo $row['type']; ?></td>
					<td><?php echo $row['schedule']; ?></td>
					<td><mark><b><?php echo $row['status']; ?></b></mark></td>
					<td><?php echo $row['adminid']; ?>&#8211;<?php echo $row['f_name']; ?><br><?php echo $row['m_name']; ?> <?php echo $row['l_name']; ?></td>
					<td>
						<a href='edit_schedule_form.php?id=<?php echo $row['id']; ?>' class='btn btn-info text-white'>Edit</a>
					</td>
					<td>
			...
			<?php
			$sql1="SELECT * FROM repairschedule";
			...
			if(mysqli_num_rows($result1) > 0){
				...
				<ul class="pagination">';
				if($page > 1){
					echo '<li ><a href="schedule_repair.php?page='.($page - 1).'">Previous</a></li>';
				}
				for($i=1; $i<= $total_page; $i++){
					...
					echo'<li class="page-item '.$active.'"><a href="schedule_repair.php?page='.$i.'" class="page-link">'.$i.'</a></li>';
				}
				if($total_page > $page){
					echo '<li ><a href="schedule_repair.php?page='.($page + 1).'">Next</a></li>';
				}
				echo'</ul>
				</div>';
			}
...
<div id="addEmployeeModal" class="modal fade">
	<div class="modal-dialog">
		...
		<!-- *********************************************************************************************** -->
        <form action="add_repair.php" method="post" onsubmit="return TDate();">
				...
				<div class="modal-body">					
					<div class="form-group">
						<?php
							...
							echo"
							<label for='select coach'>Select Coach</label>
							<select class='form-control' name='coachid' id='coachid'>";
							while($row2 = $result2->fetch_assoc()){
								echo "<option value='$row2[coachid]'>$row2[coachid]-$row2[coachtype] $row2[age] $row2[color]</option>";
							}
							echo "</select>";
							}
							else{
					...
					<div class="form-group">
						<input type="hidden" name="adminid" value="<?php $_SESSION['adminid'] ?>">
					</div>										
				</div>
